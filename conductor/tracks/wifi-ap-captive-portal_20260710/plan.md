...
- [ ] Task: Add SSID and Password to SettingsManager
    - [ ] Update `include/settings_manager.h` to declare getter/setter for SSID and Password
    - [ ] Update `src/settings_manager.cpp` to initialize, save, and load SSID and password fields from Preferences (falling back to secrets.h if not set)
- [ ] Task: Write Tests for SettingsManager Wi-Fi persistence
    - [ ] Write unit tests in `tests/test_settings/test_settings.cpp` to verify getters, setters, and default fallbacks for SSID and password
    - [ ] Run tests to verify they fail (Red Phase)
    - [ ] Complete implementation to make the tests pass (Green Phase)
